...
kernel.elf: kernel/src/main.c kernel/src/gdt.c kernel/src/gdt_flush.s \
	...
	$(LD) $(LDFLAGS) --oformat=elf64-x86-64 \
		...
		kernel/scheduler.o kernel/binary.o user_bin.o -o kernel.elf
...
user_space/user.elf: user_space/src/user_main.o user_space/src/user_task.o user_space/switch_to_user.o
	$(LD) -T user_space/linker.ld -o user_space/user.elf user_space/src/user_main.o user_space/src/user_task.o user_space/switch_to_user.o
...
user_space/switch_to_user.o: user_space/src/switch_to_user.s
...
user_bin.o: user.bin
	objcopy -I binary -O elf64-x86-64 -B i386 user.bin user_bin.o \
        --rename-section .data=.user_text,alloc,load,readonly,code \
        --add-section .user_task_text=user.bin \
...
clean:
	rm -f boot.bin kernel.elf kernel/*.o drivers/vga/*.o drivers/keyboard/*.o drivers/timer/*.o \
	      ...
	      kernel/scheduler.o \
	      scooter_os.bin \
	      user_space/src/*.o user_space/switch_to_user.o user_space/user.elf user.bin \
